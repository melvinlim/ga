...
def crossDicts(x,y):
	...
	yinds=random.sample(xrange(0,len(ykeys)),yt)
	c=dict()
	for i in xinds:
		key=xkeys[i]
		c[key]=x[key]
...
def crossDNA(x,y):
	assert(len(x)==len(y))
	result={}
	result['main']=crossInstructions(x['main'],y['main'])
	n=len(x['misc'])
	result['misc']=[]
	for i in xrange(n):
		result['misc'].append(crossInstructions(x['misc'][i],y['misc'][i]))
	result['input']={}
	keys=x['input'].keys()
	for key in keys:
